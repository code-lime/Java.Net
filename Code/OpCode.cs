...
[+] ???wide???
        */
        ...
        public override bool Equals(object obj) => obj is OpCode op && Equals(op);
        public bool Equals(OpCode obj) => obj.key == key;
        public override int GetHashCode() => (byte)key;
        public static bool operator ==(OpCode a, OpCode b) => a is OpCode op && op.Equals(b);
        public static bool operator !=(OpCode a, OpCode b) => !(a == b);
        public override string ToString() => $"{Name} (0x{Index.ToString("X2")})";
        public static implicit operator OpCode(OpCodes.Names key) => new OpCode(key);
        public static implicit operator OpCodes.Names(OpCode code) => code.key;
